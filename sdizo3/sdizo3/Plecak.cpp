...
void konwersja(int n, vector<int> &b)
{
	int i;
	for (i = 0; i <b.size(); i++)
	{
...
void losowanie_brute(int pojemnosc, int n){
	...
	StartCounter();
	for (i = 0; i <pow((double)2, (int)n); i++) //exhaustive of all possible selection to identify the optimal choice
	{
		konwersja(i, b);
		temp_w = 0;
		temp_v = 0;
		for (j = 0; j <n; j++) //loop to determine whether the current selection to meet the optimal choice * /
		if (b[j] == 1)
		{
			temp_w = temp_w + w[j];
			temp_v = temp_v + v[j];
